...
const Symptoms: React.FC = () => {
    ...
    useEffect(() => {
        if (successmsg) {
            message.success('Symptoms removed successfully');
            setShowSuccessmsg(false);
            closeModal();
            getCommonService();
        }
        if (errormsg) {
            message.error("Symptoms couldn't be removed");
    ...
    return (
        <div className="p-3 border h-100">
            <div className="d-flex mb-3 pb-1">
                <h6 className="text-dark fs-17 my-auto">Symptoms</h6>
            ...
            <Spin spinning={loading5}>
                <div className="d-flex flex-wrap">
                    {commonInfo?.symptoms && commonInfo?.symptoms?.some((item: any) => item.ischoices) ? (
                        ...
                        <div className="p-3 text-gray text-center bg-light mt-2 w-100">{loading5 ? 'Loading...' : 'No symptoms associated with this request'}</div>
                    )}
                </div>
            </Spin>
            <DiagnosisModal openModal={openModal} closeModal={closeModal} getCommonService={getCommonService} type="2" />
        </div>
    );
};
